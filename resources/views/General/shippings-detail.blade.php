          ...
          @forelse($travelDocument->items as $index => $item)
            <tr class="parent-row">
              <td style="text-align:center">
                <div class="item-no" style="margin:0 auto">{{ $item->no ?? ($index + 1) }}</div>
              </td>
              <td>
            ...
            @if(isset($item->subItems) && $item->subItems->isNotEmpty())
              @if(!empty($item->sub_item_group_title))
                <tr class="subgroup-row">
                  <td style="text-align:center">
                    <span class="sub-no">{{ $item->no ?? ($index + 1) }}.*</span>
                  </td>
                  <td colspan="7">
              ...
              @foreach($item->subItems as $sIndex => $sub)
                @php $subRowClass = $loop->last ? 'sub-row sub-group-end' : 'sub-row'; @endphp
                <tr class="{{ $subRowClass }}">
                  <td style="text-align:center">
                    <span class="sub-no">{{ $item->no ?? ($index + 1) }}.{{ $sIndex + 1 }}</span>
                  </td>
                  <td>
